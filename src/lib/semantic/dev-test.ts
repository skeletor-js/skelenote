...
const SAMPLE_CONTENT: IndexableContent[] = [
  {
    ...
    content:
      'Discussed roadmap priorities for Q4. Main focus areas include semantic search feature, sync improvements, and mobile support. Action items assigned to team members.',
  },
  {
    objectId: 'sample-2',
    title: 'Project Deadlines',
    content:
      'Task due dates for the current sprint. Frontend work due Friday, backend integration Monday. Code review scheduled for Tuesday.',
  },
  {
    objectId: 'sample-3',
    title: 'Sprint Retrospective',
    content:
      'Team retrospective notes. What went well: good collaboration, clear communication. Areas to improve: estimation accuracy, documentation.',
  },
  {
    objectId: 'sample-4',
    title: 'API Documentation',
    content:
      'REST API endpoints for the sync service. GET /sync/status returns current sync state. POST /sync/push sends local changes to server.',
  },
  {
    objectId: 'sample-5',
    title: 'Bug Fix: Login Issues',
    content:
      'Fixed authentication bug where users were logged out unexpectedly. Root cause was token expiration not being handled correctly. Added refresh token logic.',
  },
  {
    objectId: 'sample-6',
    title: 'Feature Request: Dark Mode',
    content:
      'User requested dark mode support for the application. Should respect system preferences and allow manual override. Consider implementing theme context.',
  },
  {
    objectId: 'sample-7',
    title: 'Team Sync Call Summary',
    content:
      'Weekly standup discussion points. Progress on current tasks, blockers identified, upcoming deadlines reviewed. Next meeting scheduled for Thursday.',
  },
  {
    objectId: 'sample-8',
    title: 'Database Migration Plan',
    content:
      'Steps to migrate from SQLite to PostgreSQL. Backup existing data, update connection strings, run migration scripts, verify data integrity.',
  },
];
  ...
  return {
    ...
    downloadModel: async () => {
      ...
      await eng.initialize((progress) => {
        console.log(
          `[${progress.operation}] ${progress.percent}% - ${progress.message}`
        );
      });
    ...
    chunk: (text: string, maxSize: number = 200) => {
      ...
      chunks.forEach((c, i) => {
        console.log(
          `  [${i}] (${c.text.length} chars): "${c.text.slice(0, 50)}..."`
        );
      });
      return chunks;
    },
    ...
    search: async (query: string, limit: number = 5) => {
      ...
      results.forEach((r, i) => {
        ...
        console.log(
          `  ${i + 1}. [${(r.score * 100).toFixed(1)}%] ${sample?.title ?? r.objectId}`
        );
      });
      return results;
    },
    ...
    findSimilar: async (objectId: string, limit: number = 5) => {
      ...
      const results = await eng.findSimilar(objectId, {
        limit,
        threshold: 0.3,
      });
...
export function initDevTestInterface(): void {
  ...
  if (isDev) {
    ...
    (
      window as unknown as { semanticTest: SemanticTestInterface }
    ).semanticTest = testInterface;
