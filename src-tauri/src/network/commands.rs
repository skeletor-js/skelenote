//! Tauri commands for local network sync
//!
//! These commands wrap the network module's functionality for the frontend.
//! Includes server management, mDNS discovery, peer connections, pairing, and cache management.

use super::cache::{KnownAddress, PairedDevice, PairedDevicesCache};
use super::client::PeerConnection;
use super::pairing::{get_local_ips, PairingInfo, PairingPayload};
use super::protocol::{DeviceRenamePayload, DeviceRevokePayload, MessageType};
use super::state::{ConnectedPeer, DiscoveredPeer, NetworkState};
use super::LocalSyncServer;
...
pub struct PairingState {
    ...
    pub cache: Arc<RwLock<Option<PairedDevicesCache>>>,
    /// Path to cache file
    pub cache_path: PathBuf,
}

impl PairingState {
    /// Create a new PairingState with the given cache file path
...
#[tauri::command]
pub async fn network_start_server(
    app: AppHandle,
    ...
    let fingerprint = {
        let stronghold_guard = crypto_state
            .stronghold
            .lock()
    ...
    tokio::spawn(async move {
        while let Some(event) = event_rx.recv().await {
            match event {
                super::server::ServerEvent::PeerConnected {
                    device_id,
                    device_name,
                    address,
                } => {
                ...
                super::server::ServerEvent::PeerDisconnected { device_id } => {
                    let _ = app_handle.emit(
                ...
                super::server::ServerEvent::MessageReceived {
                    device_id,
                    msg_type,
                    payload,
                } => {
                ...
                super::server::ServerEvent::Error { message } => {
                    let _ = app_handle.emit(
                        "local-sync-error",
                        serde_json::json!({
                            "message": message,
...
#[tauri::command]
pub async fn network_stop_server(network_state: State<'_, NetworkState>) -> Result<(), String> {
    let mut server = network_state.server.write().await;
...
#[tauri::command]
pub async fn network_get_server_info(
    network_state: State<'_, NetworkState>,
) -> Result<super::state::ServerInfo, String> {
    Ok(network_state.get_server_info().await)
}

/// Get list of connected peers
#[tauri::command]
pub async fn network_get_connected_peers(
    network_state: State<'_, NetworkState>,
...
#[tauri::command]
pub async fn network_get_device_info(
    network_state: State<'_, NetworkState>,
...
#[tauri::command]
pub async fn network_sync_device_id(
    crypto_state: State<'_, CryptoState>,
    network_state: State<'_, NetworkState>,
) -> Result<String, String> {
...
pub async fn network_start_discovery(
    ...
    let (handle, mut event_rx) =
        MdnsHandle::start(device_id, device_name, port, fingerprint, blocklist)?;
    ...
    tokio::spawn(async move {
        while let Some(event) = event_rx.recv().await {
            match event {
                super::mdns::MdnsEvent::PeerDiscovered(peer) => {
                ...
                super::mdns::MdnsEvent::PeerLost { device_id } => {
...
#[tauri::command]
pub async fn network_stop_discovery(network_state: State<'_, NetworkState>) -> Result<(), String> {
    // Take the handle
    let handle = {
        let mut mdns = network_state.mdns.write().await;
        mdns.handle.take()
...
#[tauri::command]
pub async fn network_get_discovered_peers(
    network_state: State<'_, NetworkState>,
...
#[tauri::command]
pub async fn network_is_discovery_running(
    network_state: State<'_, NetworkState>,
) -> Result<bool, String> {
...
#[tauri::command]
pub async fn network_connect_to_peer(
    device_id: String,
    app: tauri::AppHandle,
    network_state: State<'_, NetworkState>,
) -> Result<(), String> {
    ...
    println!(
        "[Connect] Found peer: {} at {:?}:{}",
        peer.device_name, peer.addresses, peer.port
    );

    // Check if device is blocked/revoked
    if network_state.is_device_blocked(&device_id).await {
        println!(
            "[Connect] Refusing to connect to blocked device: {}",
            device_id
        );
    ...
    let our_fingerprint = network_state
        .fingerprint
        .read()
        .await
        .clone()
        ...
        .await
        {
            Ok((connection, mut event_rx)) => {
                println!(
                    ...
                    connection.device_name, connection.device_id
                );
                ...
                network_state
                    .add_peer_write_stream_arc(peer_device_id.clone(), write_stream)
                    .await;
                println!(
                    "[Connect] Added write stream to peer_streams for {}",
                    peer_device_id
                );

                // Emit connected event
                println!(
                    "[Connect] Emitting local-peer-connected event for {}",
                    peer_device_id
                );
                match app.emit(
                    "local-peer-connected",
                ...
                tokio::spawn(async move {
                    while let Some(event) = event_rx.recv().await {
                        match event {
                            super::client::PeerEvent::MessageReceived { msg_type, payload } => {
                            ...
                            super::client::PeerEvent::Disconnected => {
                            ...
                            super::client::PeerEvent::Error { message } => {
...
#[tauri::command]
pub async fn network_broadcast_sync(
    data: Vec<u8>,
    network_state: State<'_, NetworkState>,
) -> Result<usize, String> {
...
#[tauri::command]
pub async fn network_peer_count(network_state: State<'_, NetworkState>) -> Result<usize, String> {
...
/// Returns the number of peers the data was sent to.
#[tauri::command]
pub async fn network_broadcast_device_registry(
    data: Vec<u8>,
    network_state: State<'_, NetworkState>,
) -> Result<usize, String> {
    let count = network_state
        .broadcast_raw(MessageType::DeviceRegistry, &data)
        .await;
    Ok(count)
}
...
#[tauri::command]
pub async fn network_broadcast_device_revoke(
    payload: String,
    network_state: State<'_, NetworkState>,
) -> Result<usize, String> {
    // Parse and validate the payload structure
    let revoke: DeviceRevokePayload =
        serde_json::from_str(&payload).map_err(|e| format!("Invalid revoke payload: {}", e))?;
    ...
    let count = network_state
        .broadcast_raw(MessageType::DeviceRevoke, &data)
        .await;
    Ok(count)
}
...
/// Returns the number of peers the message was sent to.
#[tauri::command]
pub async fn network_broadcast_device_rename(
    payload: String,
    network_state: State<'_, NetworkState>,
) -> Result<usize, String> {
    // Parse and validate the payload structure
    let rename: DeviceRenamePayload =
        serde_json::from_str(&payload).map_err(|e| format!("Invalid rename payload: {}", e))?;
    ...
    let count = network_state
        .broadcast_raw(MessageType::DeviceRename, &data)
        .await;
    Ok(count)
}
...
#[tauri::command]
pub async fn device_is_blocked(
    device_id: String,
    network_state: State<'_, NetworkState>,
) -> Result<bool, String> {
...
#[tauri::command]
pub async fn device_get_blocked(
    network_state: State<'_, NetworkState>,
...
async fn connect_to_peer_direct(
    ...
    // Connect
    let (connection, mut event_rx) =
        PeerConnection::connect(addr, our_device_id, our_device_name, our_fingerprint).await?;

    println!(
        "[ConnectDirect] Connected to {} ({})",
        connection.device_name, connection.device_id
    );
...
#[derive(Debug, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ManualPairingDetails {
...
pub async fn pairing_generate_qr(
    ...
    let fingerprint = {
        ...
        fp_guard
            .clone()
            .ok_or("Fingerprint not set - server must be started first")?
    };
    ...
    let port = server_info
        .port
        .ok_or("Server not running - call network_start_server first")?;
    ...
    // Create payload
    let payload_struct =
        PairingPayload::new(ips.clone(), port, fingerprint.clone(), device_id, device_name.clone());
    ...
    let formatted_code = fingerprint
        .to_uppercase()
        .chars()
        .collect::<Vec<_>>()
        .chunks(4)
...
#[tauri::command]
pub async fn pairing_connect_manual(
    ip: String,
    port: u16,
    code: String,
    app: AppHandle,
    ...
    .await
    {
        Ok(_) => {
            ...
            let peer_info = {
                ...
                peers
                    .values()
                    .find(|p| p.address == format!("{}:{}", ip, port))
                    .cloned()
            };
        ...
        Err(e) => Err(format!("Failed to connect to {}:{}: {}", ip, port, e)),
    }
}
...
#[derive(Debug, serde::Serialize)]
pub struct PairedDeviceWithStatus {
    #[serde(flatten)]
    device: PairedDevice,
    connected: bool,
}
...
#[tauri::command]
pub async fn cache_get_paired_devices(
    pairing_state: State<'_, PairingState>,
    network_state: State<'_, NetworkState>,
...
#[tauri::command]
pub async fn cache_remove_paired_device(
    device_id: String,
    pairing_state: State<'_, PairingState>,
) -> Result<(), String> {
    let mut cache_guard = pairing_state.cache.write().await;
...
#[tauri::command]
pub async fn cache_prune_addresses(pairing_state: State<'_, PairingState>) -> Result<(), String> {
    let mut cache_guard = pairing_state.cache.write().await;
    let cache = cache_guard.as_mut().ok_or("Cache not initialized")?;

    println!(
        "[Cache] Pruning dead addresses from {} devices",
        cache.devices.len()
    );
...
#[tauri::command]
pub async fn cache_reconnect_all(
    app: AppHandle,
    _crypto_state: State<'_, CryptoState>,
    network_state: State<'_, NetworkState>,
    pairing_state: State<'_, PairingState>,
    ...
    for device in devices {
        ...
        let task = tokio::spawn(async move {
            ...
            for addr_info in addresses {
                ...
                let addr: SocketAddr = match format!("{}:{}", ip, port).parse() {
                    ...
                    Err(e) => {
                        ...
                        // Record failure in cache
                        {
                            ...
                            if let Some(cache) = cache_guard.as_mut() {
                                let _ =
                                    cache.record_connection_failure(&target_device_id, &ip, port);
                            }
                        }
                        continue;
                    }
                ...
                match PeerConnection::connect(addr, dev_id.clone(), dev_name.clone(), fp.clone())
                    .await
                {
                    Ok((connection, _event_rx)) => {
                        ...
                        // Record success in cache
                        {
                            ...
                            if let Some(cache) = cache_guard.as_mut() {
                                let _ =
                                    cache.record_connection_success(&target_device_id, &ip, port);
                                // Save cache to disk
                        ...
                        println!(
                            "[Reconnect] Successfully connected to device {} at {}:{}",
                            target_device_id, ip, port
                        );
                        connected = true;
                        break;
                    }
                    Err(e) => {
                        println!(
                            "[Reconnect] Failed to connect to device {} at {}:{}: {}",
                            target_device_id, ip, port, e
                        );
            ...
            if connected {
                ...
                Err(format!(
                    "Failed to connect: {}",
                    last_error.unwrap_or_else(|| "No addresses available".to_string())
                ))
            }
        });
...
#[tauri::command]
pub async fn cache_reconnect_device(
    device_id: String,
    app: AppHandle,
    _crypto_state: State<'_, CryptoState>,
    network_state: State<'_, NetworkState>,
    ...
    for (ip, port) in &addresses {
        ...
        .await
        {
            Ok(_) => {
                println!(
                    "[Reconnect] Successfully connected to {} at {}:{}",
                    device_id, ip, port
                );
            ...
            Err(e) => {
                println!(
                    "[Reconnect] Failed to connect to {} at {}:{}: {}",
                    device_id, ip, port, e
                );
                last_error = Some(e);
    ...
    Err(format!(
        "Failed to connect to device {} - tried {} addresses. Last error: {}",
        device_id, address_count, error_msg
    ))
}
