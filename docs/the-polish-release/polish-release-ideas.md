# The Polish Release - Feature Ideas

This document captures feature ideas for enhancing Skelenote while staying true to our core principles:
- **Local-first**: All data stored on device, no cloud dependency
- **Zero-knowledge**: Only you can read your data
- **Object-based thinking**: Everything connects via typed relations
- **Works offline**: Full functionality without internet
- **No vendor lock-in**: Your data, your files
...
### 14. Skeleton Key Health & Backup
**Complexity:** Low | **Priority:** High

Ensure users don't lose access to their data.

**Key Features:**
- Periodic reminder to verify Skeleton Key backup
- "Test your backup" flow (enter key to verify it works)
- Export encrypted backup bundle (data + verification)
- Recovery instructions clearly accessible
- Optional: print-friendly Skeleton Key card

**Why it fits our ethos:** Critical for zero-knowledge architecture. We can't recover keys—users must.
...
- `g` commands: `gg` top, `G` bottom, `gd` daily notes
- `m` to create mark, `'` to jump to mark
- Optional—toggled in settings

**Why it fits our ethos:** Respects power users. All local, instant response.
...
Named, typed connections between objects.

**Key Features:**
- Define relationship types: "blocks", "is blocked by", "relates to", "parent of"
- Relationships are bidirectional with inverse labels
- Filter graph view by relationship type
- Query by relationship in saved views
- Useful for GTD workflows (task dependencies)

**Why it fits our ethos:** Deepens "object-based thinking". Richer local knowledge graph.

---

### 18. Weekly Review Dashboard
**Complexity:** Medium | **Priority:** Medium

Aggregated view of your week.

**Key Features:**
- Objects created/modified this week
- Tasks completed vs created
- Most linked-to objects (trending)
- Orphan objects (no links) to review
- Optional: streak tracking for daily notes

**Why it fits our ethos:** Reflection on your local data. No external analytics.
...
### 20. Quick Actions from Search
**Complexity:** Low | **Priority:** Medium

Act on search results without full navigation.

**Key Features:**
- Hover actions on search results: Pin, Archive, Delete, Add Tag
- Multi-select in search results for bulk actions
- "Open in Split View" from search
- Copy object link directly from search

**Why it fits our ethos:** Faster workflows, all local operations.

---

### 21. Object Templates in Quick Capture
**Complexity:** Low | **Priority:** Medium

Use templates directly from the global quick capture.

**Key Features:**
- Template selector in quick capture overlay
- Recently used templates at top
- Placeholder expansion works immediately
- Keyboard shortcut to cycle templates

**Why it fits our ethos:** Extends existing template system. Faster capture, local-only.

---

### 22. Contextual Workspaces
**Complexity:** High | **Priority:** Low

Group related objects into focused contexts.

**Key Features:**
- Create workspaces: "Work", "Personal", "Project X"
- Each workspace has its own pinned items, saved views
- Quick switch between workspaces
- Objects can belong to multiple workspaces (via tags)
- Workspace-specific settings (theme, default type)

**Why it fits our ethos:** Organize local data into mental contexts. No cloud workspace sync.

---

### 23. Content Blocks Library
**Complexity:** Medium | **Priority:** Low

Reusable content snippets across objects.

**Key Features:**
- Create named content blocks (like templates but for content chunks)
- Insert via command palette or `/` menu
- Blocks can be static (copy) or live (transclusion-like)
- Useful for: signatures, standard responses, recurring checklists

**Why it fits our ethos:** Extends transclusion concept. All content stays local.
...
| Priority | Features |
|----------|----------|
| **High** | Graph View, Quick Link `[[`, Object Preview, Import/Export, Table View, Skeleton Key Health, Local Backups |
| **Medium** | Smart Suggestions, Focus Mode, Archive, Natural Language Dates, Reminders, Command Palette, Breadcrumbs, Keyboard Mode, Relationship Types, Weekly Review, File Attachments, Quick Actions, Templates in Quick Capture |
| **Low** | Duplicate Detection, Workspaces, Content Blocks |
...
- No external API calls except for explicitly opt-in features
- AI features require existing semantic search to be enabled
- Prioritize keyboard accessibility for all new UI
- Maintain monochromatic design language (tags only color)
